...
#      },
    ...
    supported_os_families.sort.each do |os, facts|
      context "and osfamily is #{facts[:os]} and repos and apt_setting set" do
        ...
        if facts[:os] == 'Debian'
          ...
          it { should have_apt__setting_resource_count(5) }
        else
          it 'should fail' do
            expect { should contain_class('swrepo') }.to raise_error(Puppet::Error, %r{with value apt is only valid on osfamily Debian})
          end
        end
      end
    end
  ...
  supported_os_families.sort.each do |os, facts|
    ...
    describe 'when config_dir_name is not set and config_dir_purge is true' do
      ...
      unless facts[:os] == 'Debian'
        it { should contain_file(facts[:config_file]).with(
          {
            'ensure'  => 'directory',
            'recurse' => 'true',
            'purge'   => 'true',
        ...
        it { should contain_file("#{facts[:config_file]}/params-hash1.repo").with(
          {
            'require' => "File[#{facts[:config_file]}]",
          }
        )}
        it { should contain_file("#{facts[:config_file]}/params-hash2.repo").with(
          {
            'require' => "File[#{facts[:config_file]}]",
          }
        )}
        it { should contain_swrepo__repo('params-hash2').with_repotype(facts[:repotype]) }
        if facts[:os] == 'RedHat'
          it { should contain_file("#{facts[:config_file]}/redhat.repo").with(
            {
              'require' => "File[#{facts[:config_file]}]",
            }
          )}
    ...
    describe "for apt_setting" do
      ...
      context 'when apt_setting is unset' do
        context 'with apt_setting_hiera_merge set to boolean false' do
          ...
          it { should have_apt__setting_resource_count(1) }
          it { should contain_apt__setting('conf-hierafqdn').with_content('Acquire::http::proxy "https://proxy.hieradomain.tld:8080";') }
        end

        context 'with apt_setting_hiera_merge set to boolean true' do
          let(:params) { { :apt_setting_hiera_merge => true } }
          it { should have_apt__setting_resource_count(2) }
          it { should contain_apt__setting('conf-hieraosfamily').with_content('Acquire::http::proxy "http://proxy.hieradomain.tld:8080";') }
          it { should contain_apt__setting('conf-hierafqdn').with_content('Acquire::http::proxy "https://proxy.hieradomain.tld:8080";') }
        end
      end
      ...
      context 'when apt_setting is set to a valid hash' do
        context 'with apt_settinghiera_merge set to boolean false' do
          ...
          puts(:params)
          it { should have_apt__setting_resource_count(2) }
          it { should contain_apt__setting('conf-paramshttpproxy').with_content('Acquire::http::proxy "http://proxy.domain.tld:8080";') }
          it { should contain_apt__setting('conf-paramshttpsproxy').with_content('Acquire::https::proxy "https://proxy.domain.tld:8080";') }
        end

        context 'with apt_setting_hiera_merge set to boolean true' do
          let(:params) { { :apt_setting => apt_setting_hash}.merge({ :apt_setting_hiera_merge => true }) }
          it { should have_apt__setting_resource_count(2) }
          it { should_not contain_apt__setting('conf-paramshttpproxy') }
          it { should_not contain_apt__setting('conf-paramshttpsproxy') }
          it { should contain_apt__setting('conf-hieraosfamily').with_content('Acquire::http::proxy "http://proxy.hieradomain.tld:8080";') }
          it { should contain_apt__setting('conf-hierafqdn').with_content('Acquire::http::proxy "https://proxy.hieradomain.tld:8080";') }
        end
      end
  ...
  unsupported_os_families.sort.each do |os, facts|
      describe "when running on unsupported #{os}" do
        ...
        it 'should fail' do
          expect { should contain_class(subject) }.to raise_error(Puppet::Error, %r{Supported osfamilies are Debian, RedHat and Suse 11/12})
        end
      end
  end
  ...
  describe 'variable type and content validations' do
    ...
    validations = {
      ...
      'hash-apt_setting' => {
        :name    => %w[apt_setting],
        :valid   => [apt_setting_hash], # valid hashes are to complex to block test them here.
        :invalid => ['string', %w[array], 3, 2.42, true],
        :message => 'is not a Hash',
        :facts   => {:osfamily => 'Debian', :osrelease => '16.04', :lsbdistid => 'Ubuntu', :lsbdistcodename  => 'xenial'}
      },
      'string' => {
        :name    => %w[repotype config_dir_name],
        :valid   => %w[string],
    ...
    validations.sort.each do |type, var|
      let(:facts) { var[:facts] } if var[:facts]
